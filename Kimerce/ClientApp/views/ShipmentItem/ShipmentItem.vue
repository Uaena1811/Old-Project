    ...
    <div class="card">
      <div class="card-header card-header-flex">
        ...
        <div class="d-flex flex-column justify-content-center">
          <router-link :to="{path:'/shipmentItem/createorupdate/0'}" class="btn btn-primary">
            Create
          </router-link>
        </div>
        ...
        <div class="air__utils__scrollTable">
          <a-table :columns="Columns"
            ...
            <span slot="action" slot-scope="record">
              <router-link :to="{path:'/shipmentItem/details/' + record.id}" class="btn btn-sm btn-primary mr-2">
                <a-icon type="profile" />
                Chi tiết
              </router-link>
              <router-link :to="{path:'/shipmentItem/createorupdate/' + record.id}" class="btn btn-sm btn-primary mr-2">
                <a-icon type="edit" />
                Sửa
              </router-link>
              <a-button class="btn btn-sm btn-danger" @click="DeleteShipmentItem(record.id)">
                <a-icon type="delete" />
                Xóa
              </a-button>
            </span>
...
<script>
  ...
  import ShipmentItemApi from './api';
  ...
  export default {
    ...
    methods: {
      ...
      LoadData() {
        ...
        console.log(params);
        axios.post(ShipmentItemApi.list, params).then(r => {
      ...
      DeleteShipment(id) {
        if (confirm("Có thật sự muốn xóa?")) {
          axios.delete(ShipmentItemApi.delete + id).then(response => {

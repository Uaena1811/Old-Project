      ...
      <div class="card">
        <div class="card-header card-header-flex">
          <div class="d-flex flex-column justify-content-center mr-auto">
            <h6 class="mb-0">Danh sách email</h6>
          </div>
          <div class="d-flex flex-column justify-content-center">
            <router-link to="/email/createorupdate/0">
          ...
          <div class="air__utils__scrollTable">
            <a-table :columns="Columns"
              ...
              <span slot="action" slot-scope="record">
                <router-link :to="{path:'/email/createorupdate/' + record.id}" class="btn btn-sm btn-primary mr-2">
                  <a-icon type="edit" />
                  Sửa
                </router-link>
  ...
  export default {
    ...
    methods: {
      ...
      LoadData() {
        ...
        console.log(params);
        axios.post(EmailApi.list, params).then(r => {
      ...
      DeleteProduct(id) {
        if (confirm("Có thật sự muốn xóa?")) {
          axios.delete(EmailApi.delete + id).then(response => {

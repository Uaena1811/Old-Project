        ...
        <div class="air__utils__scrollTable">
          <a-table :columns="Columns"
            ...
            <span slot="action" slot-scope="record">
              <a-button class="btn btn-sm btn-primary mr-2" @click="AddRelateProduct(record.id)" icon="plus">
                Thêm
              </a-button>
            </span>
...
<script>
  ...
  import RelateProductApi from './api';
  ...
  export default {
    ...
    data() {
      return {
        ...
        addForm: {
          productIdA: this.$route.params.id,
          productIdB: null,
          id: 0,
        }
      }
    ...
    methods: {
      ...
      AddRelateProduct(id) {
        this.addForm.productIdB = id;
        axios.post(RelateProductApi.createOrUpdate, this.addForm).then(response => {
          console.log(response);
          if (response.data.result != 1) {
            this.$message.error(response.data.message);
          }
          else {
            this.$message.success("Thêm sản phẩm liên quan thành công!");
            this.LoadData();
          }
        }).catch(error => {

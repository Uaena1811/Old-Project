  ...
  <a-spin tip="Đang tải..." :spinning="IsLoading">
    <a-row>
      ...
      <div class="air__utils__heading">
        <h5>Quản lý thuộc tính đặc biệt</h5>
      </div>
      ...
      <div class="card">
        <div class="card-header card-header-flex">
          <div class="d-flex flex-column justify-content-center">
            <router-link to="/specattribute/createorupdate/0">
              <a-button class="btn btn-primary" type="primary" icon="plus">
                Tạo thuộc tính đặc biệt
              </a-button>
            </router-link>
          </div>
        </div>
          ...
          <div class="air__utils__scrollTable">
            <a-table :columns="Columns"
              ...
              <span slot="action" slot-scope="record">
                <router-link :to="{path:'/specattribute/createorupdate/' + record.id}">
  ...
  export default {
    ...
    methods: {
      ...
      LoadData() {
        ...
        console.log(params);
        axios.post(SpecAttributeApi.list, params).then(r => {
      ...
      DeleteProduct(id) {
        if (confirm("Có thật sự muốn xóa?")) {
          axios.delete(SpecAttributeApi.delete + id).then(response => {

<template>
  <div>
    <div class="air__utils__heading" style="margin-bottom:20px;">
      ...
      <h5>Tạo / cập nhật thuộc tính kho</h5>
    </div>
    <div class="row">
...
<script>
  ...
  export default {
    created() {
      Axios.get(InventoryAttributeApi.getById + this.$route.params.id).then(r => {
    ...
    methods: {
      ...
      async SaveAndFinish() {
        this.FrmProduct.validateFieldsAndScroll((errors, values) => {
          if (!errors) {
            this.GetModel();
            Axios.post(InventoryAttributeApi.createOrUpdate, this.Model).then(r => {
              if (r.data.result != 1) {
                this.$message.error(r.data.message);
              }
              else {
                this.$message.success('Lưu dữ liệu thành công', 3);
                this.$router.replace("/inventoryattribute");
              }
            }).catch(error => {

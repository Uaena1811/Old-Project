      ...
      <div class="card">
        <div class="card-header card-header-flex">
          <div class="d-flex flex-column justify-content-center mr-auto">
            <router-link :to="{ name: 'createOrUpdateCity', params: { id: 0 }}">
          ...
          <div class="air__utils__scrollTable">
            <a-table :columns="Columns"
              ...
              <span slot="action" slot-scope="record">
                <router-link :to="{ name: 'createOrUpdateCity', params: { id: record.id }}" class="btn btn-sm btn-primary mr-2">
                  <a-icon type="edit" />
                  Sửa
                </router-link>
  ...
  export default {
    ...
    methods: {
      ...
      LoadData() {
        ...
        console.log(params);
        axios.post(CityApi.list, params).then(r => {
      ...
      deleteTodo(id) {
        if (confirm("Có thật sự muốn xóa?")) {
          axios.delete(CityApi.delete + id).then(response => {
      ...
      exportToXlsx(e) {
        var params = this.GetSearchParam();
        axios.post(CityApi.exportToXlsx, params).then(response => {
          if (response) {
            window.location.href = 'https://localhost:44397/Cities/download'
          }
        });
      },
